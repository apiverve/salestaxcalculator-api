...
{
    class SalesTaxCalculatorExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/salestaxcalculator";

        /// <summary>
        /// Make a GET request to the Sales Tax Calculator API
        /// </summary>
        static async Task<JsonDocument> CallSalesTaxCalculatorAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling Sales Tax Calculator API...\n");

            var result = await CallSalesTaxCalculatorAPI();
