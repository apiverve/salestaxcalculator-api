...
class SalestaxcalculatorAPIClientError(Exception):
...
class ValidationError(SalestaxcalculatorAPIClientError):
...
class SalestaxcalculatorAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"amount": {"type": "number", "required": True, "min": 0}, "zip": {"type": "string", "required": True, "minLength": 5, "maxLength": 5}, "state": {"type": "string", "required": False, "minLength": 2, "maxLength": 2}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the SalestaxcalculatorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises SalestaxcalculatorAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/salestaxcalculator'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise SalestaxcalculatorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[SalestaxcalculatorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns SalesTaxCalculatorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises SalestaxcalculatorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise SalestaxcalculatorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise SalestaxcalculatorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise SalestaxcalculatorAPIClientError(f"Request failed: {str(e)}")
